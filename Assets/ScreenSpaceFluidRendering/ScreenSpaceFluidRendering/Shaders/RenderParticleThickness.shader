Shader "Hidden/ScreenSpaceFluidRendering/RenderParticleThickness"
{
	CGINCLUDE
	...
	struct g2f
	{
		...
		// パーティクルのデータを格納したバッファ
	StructuredBuffer<float4> _ParticlePosPressData;
    StructuredBuffer<float4> _ParticleTGRMData;
	...
	{
		v2g o = (v2g)0;
        float4 pos_press = _ParticlePosPressData[id];
        float4 tgrm = _ParticleTGRMData[id];
        ...
        if(type == 0)
        {
            o.position = float4(pos_press.xyz, 1.0);
            o.size = radius;
        }
	...
	struct fragmentOut
	{
		float  thicknessBuffer  : SV_Target0;
