...
namespace ScreenSpaceFluidRendering
{
    ...
    public class ScreenSpaceFluidRenderer : MonoBehaviour
    {
        ...
        [SerializeField]
        Shader _renderParticleThicknessShader = null;
        Material _renderParticleThicknessMaterial;
        ...
        [Header("Transparency")]
        public Camera _cameraInfo;
        public Camera _passThroughCamera;
        public RenderTexture _passThroughTexture;
        public RenderTexture _cubeMapTexture; // For computing reflection

        // 流体シミュレーションを行うスクリプトの参照
        [SerializeField]
        GPUParticleSimulation _simScript;
        ...
        void OnEnable()
        {
            // 解放処理
            CleanUp();
            _passThroughTexture.width = Screen.width;
            _passThroughTexture.height = Screen.height;

            // 計算用のマテリアルを作成
            CreateMaterial(ref _renderParticleDepthMaterial, _renderParticleDepthShader);
            CreateMaterial(ref _renderParticleThicknessMaterial, _renderParticleThicknessShader);
        ...
        void OnWillRenderObject()
        {
            ...
            else
            {
                ...
                _passThroughCamera.transform.position = _cameraInfo.transform.position;
                _passThroughCamera.transform.rotation = _cameraInfo.transform.rotation;
                _passThroughCamera.fieldOfView = _cameraInfo.fieldOfView;
                ...
                _renderParticleDepthMaterial.SetBuffer("_ParticlePosPressData", _simScript.GetPosPressBuffer());
                _renderParticleDepthMaterial.SetBuffer("_ParticleTGRMData", _simScript.GetTGRMBuffer());
                ...
                int thicknessBufferId = Shader.PropertyToID("_ThicknessBuffer");
                buf.GetTemporaryRT(thicknessBufferId, -1, -1, 24, FilterMode.Point, RenderTextureFormat.RFloat);

                buf.SetRenderTarget
                (
                    new RenderTargetIdentifier(thicknessBufferId),
                    new RenderTargetIdentifier(thicknessBufferId)
                );

                buf.ClearRenderTarget(true, true, Color.clear);

                _renderParticleThicknessMaterial.SetBuffer("_ParticlePosPressData", _simScript.GetPosPressBuffer());
                _renderParticleThicknessMaterial.SetBuffer("_ParticleTGRMData", _simScript.GetTGRMBuffer());

                buf.DrawProcedural
                (
                    Matrix4x4.identity,
                    _renderParticleThicknessMaterial,
                    0,
                    MeshTopology.Points,
                    _simScript.GetParticleNum()
                );
                ...
                buf.SetGlobalTexture("_ThicknessBuffer", thicknessBufferId);
                ...
                if(_cubeMapTexture == null)
                {
                    ...
                    buf.SetGlobalInt("_UseCubeMap", 0);
                } else
                {
                    buf.SetGlobalTexture("_PassThroughTexture", _passThroughTexture);
                    buf.SetGlobalTexture("_CubeMap", _cubeMapTexture);
                    buf.SetGlobalInt("_UseCubeMap", 1);
                    buf.SetGlobalVector("_CameraPosition", _cameraInfo.transform.position);
                    buf.SetGlobalMatrix("_InverseProjection", _cameraInfo.projectionMatrix.inverse);
                    buf.SetGlobalMatrix("_InverseView", _cameraInfo.cameraToWorldMatrix);
                    buf.SetGlobalFloat("_CameraNear", _cameraInfo.nearClipPlane);
                    buf.SetGlobalFloat("_CameraFar", _cameraInfo.farClipPlane);
                }
